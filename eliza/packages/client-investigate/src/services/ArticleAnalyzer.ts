...
import {
  ...
  ConversationMessage,
  SubtopicCluster,
} from '../types/article-generation.ts';
import type { ArticleGenerationResult, FullCluster } from '../types/index.ts';

import { Article, BackroomEntry } from '@elizaos/adapter-postgres';
...
[Updated Article Content]` + createArticleFromClusterTemplate;
  ...
  private transformToSubtopicCluster(fullCluster: FullCluster): SubtopicCluster {
    return {
      id: fullCluster.id,
      name: fullCluster.article.title, // Use article title as cluster name
      topic: fullCluster.topic,
      relatedBackrooms: fullCluster.backrooms,
      features: this.extractFeaturesFromBackrooms(fullCluster.backrooms)
    };
  }

  private extractFeaturesFromBackrooms(backrooms: BackroomEntry[]): ContentFeatures {
    // Combine features from all backrooms
    return backrooms.reduce((acc, backroom) => ({
      entities: [...new Set([...acc.entities, ...(backroom.metadata.entities || [])])],
      claims: [...new Set([...acc.claims, ...(backroom.metadata.claims || [])])],
      technicalTerms: [...new Set([...acc.technicalTerms, ...(backroom.metadata.technicalTerms || [])])]
    }), {
      entities: [],
      claims: [],
      technicalTerms: []
