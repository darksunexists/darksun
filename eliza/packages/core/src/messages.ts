...
// parse out the <think>[TEXT]</think>
// and return back an object with { answer: string, thinking: string }
// the answer is just the remaining text after the thinking, there is no <answer> tag
export function parseThinkingFromResponse(response: string): { answer: string, thinking: string } {
    const thinkingRegex = /<think>(.*?)<\/think>/s;
    const thinkingMatch = response.match(thinkingRegex);

    if (thinkingMatch) {
        // If both opening and closing tags are present, extract the thinking content
        const thinking = thinkingMatch[1].trim();
        const answer = response.replace(thinkingRegex, "").trim();
        return { answer, thinking };
    } else {
        // If only the closing tag is present, remove it and all preceding text
        const closingTagIndex = response.indexOf("</think>");
        if (closingTagIndex !== -1) {
            const answer = response.substring(closingTagIndex + 8).trim(); // +8 to skip "</think>"
            return { answer, thinking: null };
        }
    }


    // If neither opening nor closing tag is present, return the original response
    return { answer: response.trim(), thinking: null };
}