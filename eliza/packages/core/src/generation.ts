...
import { elizaLogger, parseThinkingFromResponse } from "./index.ts";
import {
    models,
    getModelSettings,
    ...
    try {
        ...
        switch (provider) {
            ...
            case ModelProviderName.NANOGPT:
                if (
                    (modelClass == ModelClass.LARGE && (runtime.character?.settings?.secrets?.LARGE_NANOGPT_MODEL.includes('deepseek') || settings.LARGE_NANOGPT_MODEL.includes('deepseek'))) ||
                    (modelClass == ModelClass.MEDIUM && (runtime.character?.settings?.secrets?.MEDIUM_NANOGPT_MODEL.includes('deepseek') || settings.MEDIUM_NANOGPT_MODEL.includes('deepseek'))) ||
                    (modelClass == ModelClass.SMALL && (runtime.character?.settings?.secrets?.SMALL_NANOGPT_MODEL.includes('deepseek') || settings.SMALL_NANOGPT_MODEL.includes('deepseek')))
                ) {
                    elizaLogger.info("Initializing Deepseek model.");
                    ...
                    elizaLogger.info("deepseekResponse:", deepseekResponse);


                    const { answer, thinking: _thinking } = parseThinkingFromResponse(deepseekResponse);
            ...
            case ModelProviderName.MISTRAL: {
                ...
                const { text: mistralResponse } = await aiGenerateText({
                    model: mistral.languageModel(model) as LanguageModel,
                    prompt: context,
                    system:
                        runtime.character.system ??
                        settings.SYSTEM_PROMPT ??
        ...
        elizaLogger.error("E.messsagerror in generateText:", error.message);
        throw error;
    }
}
...
export async function generateShouldRespond({
    ...
    while (true) {
        try {
            ...
            elizaLogger.error("Error in generateShouldRespond:", error.message);
            if (
...
export async function generateTrueOrFalse({
    ...
    while (true) {
        try {
            ...
            elizaLogger.error("Error in generateTrueOrFalse:", error.message);
        }
...
export async function generateTextArray({
    ...
    while (true) {
        try {
            ...
            elizaLogger.error("Error in generateTextArray:", error.message);
        }
...
export async function generateObjectDeprecated({
    ...
    while (true) {
        try {
            ...
            elizaLogger.error("Error in generateObject:", error.message);
        }
...
export const generateImage = async (
        ...
        runtime.imageModelProvider === runtime.modelProvider
            ...
            : (() => {
                  ...
                  switch (runtime.imageModelProvider) {
                      ...
                      case ModelProviderName.NANOGPT:
                          return runtime.getSetting("NANOGPT_API_KEY");
                      default:
    ...
    try {
        if (runtime.imageModelProvider === ModelProviderName.HEURIST) {
        ...
        } else if (runtime.imageModelProvider === ModelProviderName.NANOGPT) {
            const response = await fetch(
            ...
            const result = await response.json();
            // elizaLogger.log("result console log", result);

            // log out the key values for the result
            Object.keys(result).forEach((key) => {
                elizaLogger.log(`${key}: ${result[key]}`);
                if (key === "data") {
                    // map out keys for data, and log them as well
                    // as the type of the value
                    Object.keys(result.data).forEach((dataKey) => {
                        elizaLogger.log(`${dataKey}: ${typeof result.data[dataKey]}`);
                    });
                }
            });


            const base64s = result.data.map(
                (image) => `data:image/jpeg;base64,${image.b64_json}`
            );

            // log out the last 10 parts of the base64s of each item
            base64s.forEach((base64) => {
                elizaLogger.log(base64.slice(-10));
            });
...
export const generateObject = async ({
    ...
    try {
        ...
        console.error("Error in generateObject:", error.message);
        throw error;
    }
};
...
export async function generateTweetActions({
    ...
    while (true) {
        try {
            ...
            elizaLogger.error("Error in generateTweetActions:", error.message);
            if (
                error instanceof TypeError &&
                error.message.includes("queueTextCompletion")
            ) {
