...
//
// BUG: SpeakerRequest and SpaceConfig are not being exported from agent-twitter-client
//      even thought they should be, debug and fix
//
...
async function generateTopicsIfEmpty(
    ...
    try {
        const context = composeContext({
            // @ts-expect-error: state is not typed here
            state: {},
            template: `
# INSTRUCTIONS:
...
export class TwitterSpaceClient {
    ...
    constructor(client: ClientBase, runtime: IAgentRuntime) {
        ...
        // @ts-expect-error: character.twitterSpaces is not typed
        // must debug and fix
