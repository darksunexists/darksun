...
export async function sendTweet(
    ...
    const maxTweetLength = isInvestigate
        ? client.twitterConfig.MAX_INVESTIGATE_TWEET_LENGTH
        : client.twitterConfig.MAX_TWEET_LENGTH;
    elizaLogger.info("Max tweet length:", maxTweetLength);
...
export async function extractMediaLinksFromThread(tweetText: string, linksToIgnore: string[]): Promise<string[] | null> {
    const urlRegex = /(https?:\/\/[^\s]+)/g;
    const matches = tweetText.match(urlRegex);

    if (!matches) return null;

    // Get array of image URLs from linksToIgnore
    const imageUrls = new Set(linksToIgnore);
    
    // Find all URLs that aren't image URLs
    const nonImageUrls = matches.filter(url => !imageUrls.has(url));

    return nonImageUrls.length > 0 ? nonImageUrls : null;
}


export async function extractArticleLink(tweetText: string): Promise<string | null> {
    const urlRegex = /(https?:\/\/[^\s]+)/g;
    const matches = tweetText.match(urlRegex);
